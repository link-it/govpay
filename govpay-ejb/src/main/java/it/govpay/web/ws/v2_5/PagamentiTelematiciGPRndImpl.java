...
public class PagamentiTelematiciGPRndImpl implements PagamentiTelematiciGPRnd {
	...
	public GpRegistraIncassoResponse gpRegistraIncasso(GpRegistraIncasso bodyrichiesta) {
		...
		try {
			...
			RichiestaIncassoDTO richiestaIncassoDTO = new RichiestaIncassoDTO();
			richiestaIncassoDTO.setPrincipal(applicazione.getPrincipal());
			...
			response.setMittente(Mittente.GOV_PAY);
			response.setCodEsito(ie.getFaultCode());
			response.setDescrizioneEsito(ie.getFaultString());
			response.setDettaglioEsito(ie.getDescription());
