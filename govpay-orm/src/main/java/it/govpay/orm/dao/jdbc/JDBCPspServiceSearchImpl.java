...
public class JDBCPspServiceSearchImpl implements IJDBCServiceSearchWithId<Psp, IdPsp, JDBCServiceManager> {
	...
	public List<Psp> findAll(JDBCServiceManagerProperties jdbcProperties, Logger log, Connection connection, ISQLQueryObject sqlQueryObject, JDBCPaginatedExpression expression, org.openspcoop2.generic_project.beans.IDMappingBehaviour idMappingResolutionBehaviour) throws NotImplementedException, ServiceException,Exception {
		...
		try {
			...
			fields.add(Psp.model().COD_FLUSSO);
			fields.add(Psp.model().ABILITATO);
			fields.add(Psp.model().MARCA_BOLLO);
			fields.add(new CustomField("id_tracciato_xml", Long.class, "id_tracciato_xml", this.getPspFieldConverter().toTable(Psp.model())));
			List<Map<String, Object>> returnMap = this.select(jdbcProperties, log, connection, sqlQueryObject, expression, fields.toArray(new IField[1]));
			...
			for(Map<String, Object> map: returnMap) {
				...
				Psp psp = (Psp)this.getPspFetch().fetch(jdbcProperties.getDatabase(), Psp.model(), map);
				
                if(idMappingResolutionBehaviour==null ||
                        (org.openspcoop2.generic_project.beans.IDMappingBehaviour.ENABLED.equals(idMappingResolutionBehaviour) || org.openspcoop2.generic_project.beans.IDMappingBehaviour.USE_TABLE_ID.equals(idMappingResolutionBehaviour))
                        ){
                it.govpay.orm.IdTracciato id_psp_tracciatoXML = null;
                if(idMappingResolutionBehaviour==null || org.openspcoop2.generic_project.beans.IDMappingBehaviour.ENABLED.equals(idMappingResolutionBehaviour)){
                        id_psp_tracciatoXML = ((JDBCTracciatoXMLServiceSearch)(this.getServiceManager().getTracciatoXMLServiceSearch())).findId(idTracciato, false);
                }else{
                        id_psp_tracciatoXML = new it.govpay.orm.IdTracciato();
                }
                id_psp_tracciatoXML.setId(idTracciato);
                psp.setIdTracciato(id_psp_tracciatoXML);
        }
	...
	private void _join(IExpression expression, ISQLQueryObject sqlQueryObject) throws NotImplementedException, ServiceException, Exception{
		...
		if(expression.inUseModel(Psp.model().ID_TRACCIATO,false)){
			String tableName1 = this.getPspFieldConverter().toAliasTable(Psp.model());
			String tableName2 = this.getPspFieldConverter().toAliasTable(Psp.model().ID_TRACCIATO);
	...
	protected Map<String, List<IField>> _getMapTableToPKColumn() throws NotImplementedException, Exception{
		...
		// Psp.model().ID_TRACCIATO
		mapTableToPKColumn.put(converter.toTable(Psp.model().ID_TRACCIATO),
				utilities.newList(
						new CustomField("id", Long.class, "id", converter.toTable(Psp.model().ID_TRACCIATO))
						));
