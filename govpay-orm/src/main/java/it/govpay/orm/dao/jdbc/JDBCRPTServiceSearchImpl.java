...
public class JDBCRPTServiceSearchImpl implements IJDBCServiceSearchWithId<RPT, IdRpt, JDBCServiceManager> {
	...
	public List<RPT> findAll(JDBCServiceManagerProperties jdbcProperties, Logger log, Connection connection, ISQLQueryObject sqlQueryObject, JDBCPaginatedExpression expression, org.openspcoop2.generic_project.beans.IDMappingBehaviour idMappingResolutionBehaviour) throws NotImplementedException, ServiceException,Exception {
		...
		try{
			...
			fields.add(RPT.model().DATA_ORA_MSG_RICHIESTA);
			fields.add(RPT.model().DATA_ORA_CREAZIONE);
			fields.add(RPT.model().COD_MSG_RICHIESTA);
			fields.add(RPT.model().IBAN_ADDEBITO);
			fields.add(RPT.model().AUTENTICAZIONE_SOGGETTO);
			fields.add(RPT.model().FIRMA_RT);
			fields.add(RPT.model().STATO);
			fields.add(RPT.model().DESCRIZIONE_STATO);
			fields.add(RPT.model().COD_FAULT);
			...
			fields.add(new CustomField("id_tracciato_xml", Long.class, "id_tracciato_xml", this.getRPTFieldConverter().toTable(RPT.model())));
			fields.add(new CustomField("id_anagrafica_versante", Long.class, "id_anagrafica_versante", this.getRPTFieldConverter().toTable(RPT.model())));
			fields.add(new CustomField("id_psp", Long.class, "id_psp", this.getRPTFieldConverter().toTable(RPT.model())));
			fields.add(new CustomField("id_portale", Long.class, "id_portale", this.getRPTFieldConverter().toTable(RPT.model())));
			fields.add(new CustomField("id_stazione", Long.class, "id_stazione", this.getRPTFieldConverter().toTable(RPT.model())));
			...
			for(Map<String, Object> map: returnMap) {
				...
				Long idTracciato = getNullableValueFromMap("id_tracciato_xml", map);
				Long idStazione = getNullableValueFromMap("id_stazione", map);
				Long idAnagrafica = getNullableValueFromMap("id_anagrafica_versante", map);;

				Long idPsp = getNullableValueFromMap("id_psp", map);
				Long idCanale = getNullableValueFromMap("id_canale", map);
				...
				if(idTracciato != null && idTracciato > 0){
					it.govpay.orm.IdTracciato id_rpt_tracciatoXML = null;
					if(idMappingResolutionBehaviour==null || org.openspcoop2.generic_project.beans.IDMappingBehaviour.ENABLED.equals(idMappingResolutionBehaviour)){
						id_rpt_tracciatoXML = ((JDBCTracciatoXMLServiceSearch)(this.getServiceManager().getTracciatoXMLServiceSearch())).findId(idTracciato, false);
					}else{
						id_rpt_tracciatoXML = new it.govpay.orm.IdTracciato();
					}
					id_rpt_tracciatoXML.setId(idTracciato);
					rpt.setIdTracciatoXML(id_rpt_tracciatoXML);
				}

				if(idPsp != null && idPsp > 0){
					it.govpay.orm.IdPsp id_rpt_psp = null;
					if(idMappingResolutionBehaviour==null || org.openspcoop2.generic_project.beans.IDMappingBehaviour.ENABLED.equals(idMappingResolutionBehaviour)){
						id_rpt_psp = ((JDBCPspServiceSearch)(this.getServiceManager().getPspServiceSearch())).findId(idPsp, false);
					}else{
						id_rpt_psp = new it.govpay.orm.IdPsp();
					}
					id_rpt_psp.setId(idPsp);
					rpt.setIdPsp(id_rpt_psp);
				}

				if(idCanale != null && idCanale > 0){
				...
				if(idAnagrafica != null && idAnagrafica > 0) {
					it.govpay.orm.IdAnagrafica id_rpt_anagrafica = null;
					if(idMappingResolutionBehaviour==null || org.openspcoop2.generic_project.beans.IDMappingBehaviour.ENABLED.equals(idMappingResolutionBehaviour)){
						id_rpt_anagrafica = ((JDBCAnagraficaServiceSearch)(this.getServiceManager().getAnagraficaServiceSearch())).findId(idAnagrafica, false);
					}else{
						id_rpt_anagrafica = new it.govpay.orm.IdAnagrafica();
					}
					id_rpt_anagrafica.setId(idAnagrafica);
					rpt.setIdAnagraficaVersante(id_rpt_anagrafica);

				}

				if(idStazione != null && idStazione > 0){
					it.govpay.orm.IdStazione id_rpt_stazione = null;
					if(idMappingResolutionBehaviour==null || org.openspcoop2.generic_project.beans.IDMappingBehaviour.ENABLED.equals(idMappingResolutionBehaviour)){
						id_rpt_stazione = ((JDBCStazioneServiceSearch)(this.getServiceManager().getStazioneServiceSearch())).findId(idStazione, false);
					}else{
						id_rpt_stazione = new it.govpay.orm.IdStazione();
					}
					id_rpt_stazione.setId(idStazione);
					rpt.setIdStazione(id_rpt_stazione);
				}
	...
	private void _mappingTableIds(JDBCServiceManagerProperties jdbcProperties, Logger log, Connection connection, ISQLQueryObject sqlQueryObject, RPT obj, RPT imgSaved) throws NotFoundException,NotImplementedException,ServiceException,Exception{
		...
		if(obj.getIdAnagraficaVersante()!=null && 
				imgSaved.getIdAnagraficaVersante()!=null){
			obj.getIdAnagraficaVersante().setId(imgSaved.getIdAnagraficaVersante().getId());
		}
	...
	private void _join(IExpression expression, ISQLQueryObject sqlQueryObject) throws NotImplementedException, ServiceException, Exception{
		...
		if(expression.inUseModel(RPT.model().ID_ANAGRAFICA_VERSANTE,false)){
			String tableName1 = this.getRPTFieldConverter().toAliasTable(RPT.model());
			String tableName2 = this.getRPTFieldConverter().toAliasTable(RPT.model().ID_ANAGRAFICA_VERSANTE);
			sqlQueryObject.addWhereCondition(tableName1+".id_anagrafica_versante="+tableName2+".id");
		}

		if(expression.inUseModel(RPT.model().ID_TRACCIATO_XML,false)){
			String tableName1 = this.getRPTFieldConverter().toAliasTable(RPT.model());
			String tableName2 = this.getRPTFieldConverter().toAliasTable(RPT.model().ID_TRACCIATO_XML);
		...
		if(expression.inUseModel(RPT.model().ID_PSP,false)){
			String tableName1 = this.getRPTFieldConverter().toAliasTable(RPT.model());
			String tableName2 = this.getRPTFieldConverter().toAliasTable(RPT.model().ID_PSP);
		...
		if(expression.inUseModel(RPT.model().ID_STAZIONE,false)){
			String tableName1 = this.getRPTFieldConverter().toAliasTable(RPT.model());
			String tableName2 = this.getRPTFieldConverter().toAliasTable(RPT.model().ID_STAZIONE);
	...
	protected Map<String, List<IField>> _getMapTableToPKColumn() throws NotImplementedException, Exception{
		...
		// RPT.model().ID_PSP
		mapTableToPKColumn.put(converter.toTable(RPT.model().ID_PSP),
				utilities.newList(
						new CustomField("id", Long.class, "id", converter.toTable(RPT.model().ID_PSP))
						));
		...
		// RPT.model().ID_TRACCIATO_XML
		mapTableToPKColumn.put(converter.toTable(RPT.model().ID_TRACCIATO_XML),
				utilities.newList(
						new CustomField("id", Long.class, "id", converter.toTable(RPT.model().ID_TRACCIATO_XML))
						));

		// RPT.model().ID_STAZIONE
		mapTableToPKColumn.put(converter.toTable(RPT.model().ID_STAZIONE),
				utilities.newList(
						new CustomField("id", Long.class, "id", converter.toTable(RPT.model().ID_STAZIONE))
						));
		// RPT.model().ID_ANAGRAFICA_VERSANTE
		mapTableToPKColumn.put(converter.toTable(RPT.model().ID_ANAGRAFICA_VERSANTE),
				utilities.newList(
						new CustomField("id", Long.class, "id", converter.toTable(RPT.model().ID_ANAGRAFICA_VERSANTE))
						));
