...
public class JDBCRendicontazionePagamentoServiceSearchImpl implements IJDBCServiceSearchWithoutId<RendicontazionePagamento, JDBCServiceManager> {
	...
	public IJDBCFetch getFetch() {
		return getRendicontazionePagamentoFetch();
	}
	...
	public List<RendicontazionePagamento> findAll(JDBCServiceManagerProperties jdbcProperties, Logger log, Connection connection, ISQLQueryObject sqlQueryObject, JDBCPaginatedExpression expression, org.openspcoop2.generic_project.beans.IDMappingBehaviour idMappingResolutionBehaviour) throws NotImplementedException, ServiceException,Exception {
        ...
        List<RendicontazionePagamento> list = new ArrayList<RendicontazionePagamento>();
        
        try{
			List<IField> fields = new ArrayList<IField>();
			fields.add(new CustomField("FR.id", Long.class, "id", this.getFieldConverter().toTable(RendicontazionePagamento.model().FR)));
			fields.add(new CustomField("id_psp", Long.class, "id_psp", this.getFieldConverter().toTable(RendicontazionePagamento.model().FR)));
			fields.add(new CustomField("id_dominio", Long.class, "id_dominio", this.getFieldConverter().toTable(RendicontazionePagamento.model().FR)));
			...
			fields.add(new CustomField("FrApplicazione.id", Long.class, "id", this.getFieldConverter().toTable(RendicontazionePagamento.model().FR_APPLICAZIONE)));
			fields.add(new CustomField("id_applicazione", Long.class, "id_applicazione", this.getFieldConverter().toTable(RendicontazionePagamento.model().FR_APPLICAZIONE)));
			fields.add(RendicontazionePagamento.model().FR_APPLICAZIONE.NUMERO_PAGAMENTI);
			fields.add(RendicontazionePagamento.model().FR_APPLICAZIONE.IMPORTO_TOTALE_PAGAMENTI);
			...
			fields.add(RendicontazionePagamento.model().PAGAMENTO.COD_SINGOLO_VERSAMENTO_ENTE);
			...
			fields.add(RendicontazionePagamento.model().PAGAMENTO.RENDICONTAZIONE_ESITO);
			fields.add(RendicontazionePagamento.model().PAGAMENTO.RENDICONTAZIONE_DATA);
			fields.add(RendicontazionePagamento.model().PAGAMENTO.CODFLUSSO_RENDICONTAZIONE);
			fields.add(RendicontazionePagamento.model().PAGAMENTO.ANNO_RIFERIMENTO);
			fields.add(RendicontazionePagamento.model().PAGAMENTO.INDICE_SINGOLO_PAGAMENTO);
			...
			fields.add(RendicontazionePagamento.model().PAGAMENTO.RENDICONTAZIONE_ESITO_REVOCA);
			fields.add(RendicontazionePagamento.model().PAGAMENTO.RENDICONTAZIONE_DATA_REVOCA);
			fields.add(RendicontazionePagamento.model().PAGAMENTO.COD_FLUSSO_RENDICONTAZIONE_REVOCA);
			fields.add(RendicontazionePagamento.model().PAGAMENTO.ANNO_RIFERIMENTO_REVOCA);
			fields.add(RendicontazionePagamento.model().PAGAMENTO.INDICE_SINGOLO_PAGAMENTO_REVOCA);
			...
			for(Map<String, Object> map: returnMap) {
				...
				FrApplicazione frApplicazione = (FrApplicazione) this.getFetch().fetch(jdbcProperties.getDatabase(), RendicontazionePagamento.model().FR_APPLICAZIONE, map);
				...
				rendicontazionePagamento.setFrApplicazione(frApplicazione);
...
//						(org.openspcoop2.generic_project.beans.IDMappingBehaviour.ENABLED.equals(idMappingResolutionBehaviour) || org.openspcoop2.generic_project.beans.IDMappingBehaviour.USE_TABLE_ID.equals(idMappingResolutionBehaviour))
//					){
//						// Object _rendicontazionePagamento_pagamento_frApplicazione (recupero id)
//						ISQLQueryObject sqlQueryObjectGet_rendicontazionePagamento_pagamento_frApplicazione_readFkId = sqlQueryObjectGet.newSQLQueryObject();
//						sqlQueryObjectGet_rendicontazionePagamento_pagamento_frApplicazione_readFkId.addFromTable(this.getRendicontazionePagamentoFieldConverter().toTable(it.govpay.orm.RendicontazionePagamento.model().PAGAMENTO));
//						sqlQueryObjectGet_rendicontazionePagamento_pagamento_frApplicazione_readFkId.addSelectField("id_fr_applicazione");
//						sqlQueryObjectGet_rendicontazionePagamento_pagamento_frApplicazione_readFkId.addWhereCondition("id=?");
//						sqlQueryObjectGet_rendicontazionePagamento_pagamento_frApplicazione_readFkId.setANDLogicOperator(true);
//						Long idFK_rendicontazionePagamento_pagamento_frApplicazione = (Long) jdbcUtilities.executeQuerySingleResult(sqlQueryObjectGet_rendicontazionePagamento_pagamento_frApplicazione_readFkId.createSQLQuery(), jdbcProperties.isShowSql(),Long.class,
//								new JDBCObject(rendicontazionePagamento_pagamento.getId(),Long.class));
//						
//						it.govpay.orm.IdFrApplicazione id_rendicontazionePagamento_pagamento_frApplicazione = null;
//						if(idMappingResolutionBehaviour==null || org.openspcoop2.generic_project.beans.IDMappingBehaviour.ENABLED.equals(idMappingResolutionBehaviour)){
//							id_rendicontazionePagamento_pagamento_frApplicazione = ((JDBCFrApplicazioneServiceSearch)(this.getServiceManager().getFrApplicazioneServiceSearch())).findId(idFK_rendicontazionePagamento_pagamento_frApplicazione, false);
//						}else{
//							id_rendicontazionePagamento_pagamento_frApplicazione = new it.govpay.orm.IdFrApplicazione();
//						}
//						id_rendicontazionePagamento_pagamento_frApplicazione.setId(idFK_rendicontazionePagamento_pagamento_frApplicazione);
//						//TODO Impostare il corretto metodo che contiene l'identificativo logico
//						//rendicontazionePagamento_pagamento.setFrApplicazione(id_rendicontazionePagamento_pagamento_frApplicazione);
//					}
...
//						ISQLQueryObject sqlQueryObjectGet_rendicontazionePagamento_pagamento_frApplicazione_readFkId = sqlQueryObjectGet.newSQLQueryObject();
//						sqlQueryObjectGet_rendicontazionePagamento_pagamento_frApplicazione_readFkId.addFromTable(this.getRendicontazionePagamentoFieldConverter().toTable(it.govpay.orm.RendicontazionePagamento.model().PAGAMENTO));
//						sqlQueryObjectGet_rendicontazionePagamento_pagamento_frApplicazione_readFkId.addSelectField("id_fr_applicazione_revoca");
//						sqlQueryObjectGet_rendicontazionePagamento_pagamento_frApplicazione_readFkId.addWhereCondition("id=?");
//						sqlQueryObjectGet_rendicontazionePagamento_pagamento_frApplicazione_readFkId.setANDLogicOperator(true);
//						Long idFK_rendicontazionePagamento_pagamento_frApplicazione = (Long) jdbcUtilities.executeQuerySingleResult(sqlQueryObjectGet_rendicontazionePagamento_pagamento_frApplicazione_readFkId.createSQLQuery(), jdbcProperties.isShowSql(),Long.class,
//								new JDBCObject(rendicontazionePagamento_pagamento.getId(),Long.class));
...
//					}
	...
	private void _join(IExpression expression, ISQLQueryObject sqlQueryObject) throws NotImplementedException, ServiceException, Exception{
		...
		String frApp = this.getRendicontazionePagamentoFieldConverter().toAliasTable(RendicontazionePagamento.model().FR_APPLICAZIONE);
		...
		sqlQueryObject.addWhereCondition(frApp+".id="+pagamento+".id_fr_applicazione");
		...
		if(expression.inUseModel(RendicontazionePagamento.model().FR_APPLICAZIONE.ID_APPLICAZIONE,false)){
			String tableName1 = this.getFieldConverter().toAliasTable(RendicontazionePagamento.model().FR_APPLICAZIONE);
			String tableName2 = this.getFieldConverter().toAliasTable(RendicontazionePagamento.model().FR_APPLICAZIONE.ID_APPLICAZIONE);
			sqlQueryObject.addWhereCondition(tableName1+".id_applicazione="+tableName2+".id");
		}

		if(expression.inUseModel(RendicontazionePagamento.model().FR.ID_DOMINIO,false)){
			String tableName1 = this.getFieldConverter().toAliasTable(RendicontazionePagamento.model().FR);
			String tableName2 = this.getFieldConverter().toAliasTable(RendicontazionePagamento.model().FR.ID_DOMINIO);
			sqlQueryObject.addWhereCondition(tableName1+".id_dominio="+tableName2+".id");
		}

		if(expression.inUseModel(RendicontazionePagamento.model().FR.ID_PSP,false)){
			String tableName1 = this.getFieldConverter().toAliasTable(RendicontazionePagamento.model().FR);
			String tableName2 = this.getFieldConverter().toAliasTable(RendicontazionePagamento.model().FR.ID_PSP);
	...
	protected Map<String, List<IField>> _getMapTableToPKColumn() throws NotImplementedException, Exception{
		...
		// RendicontazionePagamento.model().FR.ID_PSP
		mapTableToPKColumn.put(converter.toTable(RendicontazionePagamento.model().FR.ID_PSP),
			utilities.newList(
				new CustomField("id", Long.class, "id", converter.toTable(RendicontazionePagamento.model().FR.ID_PSP))
			));

		// RendicontazionePagamento.model().FR.ID_DOMINIO
		mapTableToPKColumn.put(converter.toTable(RendicontazionePagamento.model().FR.ID_DOMINIO),
			utilities.newList(
				new CustomField("id", Long.class, "id", converter.toTable(RendicontazionePagamento.model().FR.ID_DOMINIO))
			));

		// RendicontazionePagamento.model().FR_APPLICAZIONE
		mapTableToPKColumn.put(converter.toTable(RendicontazionePagamento.model().FR_APPLICAZIONE),
			utilities.newList(
				new CustomField("id", Long.class, "id", converter.toTable(RendicontazionePagamento.model().FR_APPLICAZIONE))
			));

		// RendicontazionePagamento.model().FR_APPLICAZIONE.ID_APPLICAZIONE
		mapTableToPKColumn.put(converter.toTable(RendicontazionePagamento.model().FR_APPLICAZIONE.ID_APPLICAZIONE),
			utilities.newList(
				new CustomField("id", Long.class, "id", converter.toTable(RendicontazionePagamento.model().FR_APPLICAZIONE.ID_APPLICAZIONE))
			));

		// RendicontazionePagamento.model().FR_APPLICAZIONE.ID_FR
		mapTableToPKColumn.put(converter.toTable(RendicontazionePagamento.model().FR_APPLICAZIONE.ID_FR),
			utilities.newList(
				new CustomField("id", Long.class, "id", converter.toTable(RendicontazionePagamento.model().FR_APPLICAZIONE.ID_FR))
			));
		...
		// RendicontazionePagamento.model().PAGAMENTO.ID_FR_APPLICAZIONE
		mapTableToPKColumn.put(converter.toTable(RendicontazionePagamento.model().PAGAMENTO.ID_FR_APPLICAZIONE),
			utilities.newList(
				new CustomField("id", Long.class, "id", converter.toTable(RendicontazionePagamento.model().PAGAMENTO.ID_FR_APPLICAZIONE))
			));

		// RendicontazionePagamento.model().PAGAMENTO.ID_FR_APPLICAZIONE.ID_FR
		mapTableToPKColumn.put(converter.toTable(RendicontazionePagamento.model().PAGAMENTO.ID_FR_APPLICAZIONE.ID_FR),
			utilities.newList(
				new CustomField("id", Long.class, "id", converter.toTable(RendicontazionePagamento.model().PAGAMENTO.ID_FR_APPLICAZIONE.ID_FR))
			));

		// RendicontazionePagamento.model().PAGAMENTO.ID_FR_APPLICAZIONE.ID_APPLICAZIONE
		mapTableToPKColumn.put(converter.toTable(RendicontazionePagamento.model().PAGAMENTO.ID_FR_APPLICAZIONE.ID_APPLICAZIONE),
			utilities.newList(
				new CustomField("id", Long.class, "id", converter.toTable(RendicontazionePagamento.model().PAGAMENTO.ID_FR_APPLICAZIONE.ID_APPLICAZIONE))
			));
		...
		// RendicontazionePagamento.model().PAGAMENTO.ID_FR_APPLICAZIONE_REVOCA
		mapTableToPKColumn.put(converter.toTable(RendicontazionePagamento.model().PAGAMENTO.ID_FR_APPLICAZIONE_REVOCA),
			utilities.newList(
				new CustomField("id", Long.class, "id", converter.toTable(RendicontazionePagamento.model().PAGAMENTO.ID_FR_APPLICAZIONE_REVOCA))
			));

		// RendicontazionePagamento.model().PAGAMENTO.ID_FR_APPLICAZIONE_REVOCA.ID_FR
		mapTableToPKColumn.put(converter.toTable(RendicontazionePagamento.model().PAGAMENTO.ID_FR_APPLICAZIONE_REVOCA.ID_FR),
			utilities.newList(
				new CustomField("id", Long.class, "id", converter.toTable(RendicontazionePagamento.model().PAGAMENTO.ID_FR_APPLICAZIONE_REVOCA.ID_FR))
			));

		// RendicontazionePagamento.model().PAGAMENTO.ID_FR_APPLICAZIONE_REVOCA.ID_APPLICAZIONE
		mapTableToPKColumn.put(converter.toTable(RendicontazionePagamento.model().PAGAMENTO.ID_FR_APPLICAZIONE_REVOCA.ID_APPLICAZIONE),
			utilities.newList(
				new CustomField("id", Long.class, "id", converter.toTable(RendicontazionePagamento.model().PAGAMENTO.ID_FR_APPLICAZIONE_REVOCA.ID_APPLICAZIONE))
			));
