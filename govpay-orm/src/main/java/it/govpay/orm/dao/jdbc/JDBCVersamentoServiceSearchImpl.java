...
public class JDBCVersamentoServiceSearchImpl implements IJDBCServiceSearchWithId<Versamento, IdVersamento, JDBCServiceManager> {
	...
	public IdVersamento convertToId(JDBCServiceManagerProperties jdbcProperties, Logger log, Connection connection, ISQLQueryObject sqlQueryObject, Versamento versamento) throws NotImplementedException, ServiceException, Exception{
		...
		idVersamento.setCodDominio(versamento.getCodDominio());
	...
	public List<IdVersamento> findAllIds(JDBCServiceManagerProperties jdbcProperties, Logger log, Connection connection, ISQLQueryObject sqlQueryObject, JDBCPaginatedExpression expression, org.openspcoop2.generic_project.beans.IDMappingBehaviour idMappingResolutionBehaviour) throws NotImplementedException, ServiceException,Exception {
		...
		try{
			...
			fields.add(Versamento.model().COD_DOMINIO);
	...
	public List<Versamento> findAll(JDBCServiceManagerProperties jdbcProperties, Logger log, Connection connection, ISQLQueryObject sqlQueryObject, JDBCPaginatedExpression expression, org.openspcoop2.generic_project.beans.IDMappingBehaviour idMappingResolutionBehaviour) throws NotImplementedException, ServiceException,Exception {
		...
		try{
			...
			fields.add(Versamento.model().COD_DOMINIO);
			fields.add(Versamento.model().COD_VERSAMENTO_ENTE);
			fields.add(Versamento.model().IUV);
			...
			fields.add(Versamento.model().STATO_RENDICONTAZIONE);
			...
			fields.add(new CustomField("id_anagrafica_debitore", Long.class, "id_anagrafica_debitore", this.getVersamentoFieldConverter().toTable(Versamento.model())));
			fields.add(new CustomField("id_ente", Long.class, "id_ente", this.getVersamentoFieldConverter().toTable(Versamento.model())));

			List<Map<String, Object>> returnMap = this.select(jdbcProperties, log, connection, sqlQueryObject, expression, fields.toArray(new IField[1]));

			for(Map<String, Object> map: returnMap) {
				Long idApplicazione = (Long)map.remove("id_applicazione");
				Long idAnagrafica = (Long)map.remove("id_anagrafica_debitore");
				...
				it.govpay.orm.IdAnagrafica id_versamento_anagrafica = null;
				if(idMappingResolutionBehaviour==null || org.openspcoop2.generic_project.beans.IDMappingBehaviour.ENABLED.equals(idMappingResolutionBehaviour)){
					id_versamento_anagrafica = ((JDBCAnagraficaServiceSearch)(this.getServiceManager().getAnagraficaServiceSearch())).findId(idAnagrafica, false);
				}else{
					id_versamento_anagrafica = new it.govpay.orm.IdAnagrafica();
				}
				id_versamento_anagrafica.setId(idAnagrafica);
				versamento.setIdAnagraficaDebitore(id_versamento_anagrafica);
				
				it.govpay.orm.IdEnte id_versamento_ente = null;
				if(idMappingResolutionBehaviour==null || org.openspcoop2.generic_project.beans.IDMappingBehaviour.ENABLED.equals(idMappingResolutionBehaviour)){
					id_versamento_ente = ((JDBCEnteServiceSearch)(this.getServiceManager().getEnteServiceSearch())).findId(idEnte, false);
				}else{
					id_versamento_ente = new it.govpay.orm.IdEnte();
				}
				id_versamento_ente.setId(idEnte);
				versamento.setIdEnte(id_versamento_ente);
	...
	private void _mappingTableIds(JDBCServiceManagerProperties jdbcProperties, Logger log, Connection connection, ISQLQueryObject sqlQueryObject, Versamento obj, Versamento imgSaved) throws NotFoundException,NotImplementedException,ServiceException,Exception{
		...
		if(obj.getIdAnagraficaDebitore()!=null && 
				imgSaved.getIdAnagraficaDebitore()!=null){
			obj.getIdAnagraficaDebitore().setId(imgSaved.getIdAnagraficaDebitore().getId());
		}
	...
	private void _join(IExpression expression, ISQLQueryObject sqlQueryObject) throws NotImplementedException, ServiceException, Exception{
		...
		if(expression.inUseModel(Versamento.model().ID_ANAGRAFICA_DEBITORE,false)){
			String tableName1 = this.getVersamentoFieldConverter().toAliasTable(Versamento.model());
			String tableName2 = this.getVersamentoFieldConverter().toAliasTable(Versamento.model().ID_ANAGRAFICA_DEBITORE);
			sqlQueryObject.addWhereCondition(tableName1+".id_anagrafica_debitore="+tableName2+".id");
		}

		if(expression.inUseModel(Versamento.model().ID_ENTE,false)){
			String tableName1 = this.getVersamentoFieldConverter().toAliasTable(Versamento.model());
			String tableName2 = this.getVersamentoFieldConverter().toAliasTable(Versamento.model().ID_ENTE);
	...
	protected Map<String, List<IField>> _getMapTableToPKColumn() throws NotImplementedException, Exception{
		...
		// Versamento.model().ID_ENTE
		mapTableToPKColumn.put(converter.toTable(Versamento.model().ID_ENTE),
			utilities.newList(
				new CustomField("id", Long.class, "id", converter.toTable(Versamento.model().ID_ENTE))
			));
		...
		// Versamento.model().ID_ANAGRAFICA_DEBITORE
		mapTableToPKColumn.put(converter.toTable(Versamento.model().ID_ANAGRAFICA_DEBITORE),
			utilities.newList(
				new CustomField("id", Long.class, "id", converter.toTable(Versamento.model().ID_ANAGRAFICA_DEBITORE))
			));
	...
	public IdVersamento findId(JDBCServiceManagerProperties jdbcProperties, Logger log, Connection connection, ISQLQueryObject sqlQueryObject, long tableId, boolean throwNotFound)
		...
		sqlQueryObjectGet.addSelectField(this.getVersamentoFieldConverter().toColumn(Versamento.model().COD_DOMINIO,true));
		...
		listaFieldIdReturnType_versamento.add(Versamento.model().COD_DOMINIO.getFieldType());
		...
		if(listaFieldId_versamento==null || listaFieldId_versamento.size()<=0){
			...
			id_versamento.setCodDominio((String)listaFieldId_versamento.get(0));
	...
	protected Long findIdVersamento(JDBCServiceManagerProperties jdbcProperties, Logger log, Connection connection, ISQLQueryObject sqlQueryObject, IdVersamento id, boolean throwNotFound) throws NotFoundException, ServiceException, NotImplementedException, Exception {
		...
		sqlQueryObjectGet.addWhereCondition(this.getVersamentoFieldConverter().toColumn(Versamento.model().COD_DOMINIO, true)+"=?");
		sqlQueryObjectGet.addWhereCondition(this.getVersamentoFieldConverter().toColumn(Versamento.model().COD_VERSAMENTO_ENTE, true)+"=?");

		// Recupero _versamento
		org.openspcoop2.generic_project.dao.jdbc.utils.JDBCObject [] searchParams_versamento = new org.openspcoop2.generic_project.dao.jdbc.utils.JDBCObject [] { 
				new org.openspcoop2.generic_project.dao.jdbc.utils.JDBCObject(id.getCodDominio(),Versamento.model().COD_DOMINIO.getFieldType()),
