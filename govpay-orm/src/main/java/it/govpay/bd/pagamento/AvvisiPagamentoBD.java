...
public class AvvisiPagamentoBD extends BasicBD{
	...
	public AvvisoPagamento getAvviso(String codDominio, String iuv) throws ServiceException, NotFoundException {
		try {
			...
			exp.addOrder(it.govpay.orm.Avviso.model().DATA_CREAZIONE,SortOrder.DESC);

			IPaginatedExpression pagExpr = this.getAvvisoService().toPaginatedExpression(exp);
			pagExpr.limit(1);

			List<it.govpay.orm.Avviso> avvisoVOLst = this.getAvvisoService().findAll(pagExpr);

			if(avvisoVOLst == null || avvisoVOLst.size() == 0)
				throw new NotFoundException();
			else
				if(avvisoVOLst.size() > 1)
					throw new MultipleResultException("More than one result found (result: "+avvisoVOLst.size()+")");
				else {
					it.govpay.orm.Avviso avvisoVO = avvisoVOLst.get(0);
					return AvvisoPagamentoConverter.toDTO(avvisoVO);
				}
		} catch (NotImplementedException e) {
			throw new ServiceException(e);
