...
public abstract class Versionabile implements Serializable {
	...
	public enum Versione {
		...
		public static String[] labels = {"SOAP_1","SOAP_3","REST_1","REST_2"};
		...
		public static Versione toEnum(String versione) throws IllegalArgumentException {
			return toEnum(versione.split("_")[0],versione.split("_")[1]);
		}
		
		public static Versione toEnum(String api, String label) throws IllegalArgumentException {
			for(Versione p : Versione.values()){
				if(p.getLabel().equals(label) && p.getApi().equals(api)) 
					return p;
			}
				
			throw new IllegalArgumentException("Codifica inesistente per Versione. Valore fornito [" + api + "-" + label + "] valori possibili " + ArrayUtils.toString(labels));
		}
		...
		public int compareVersione(Versione other, boolean ignoreApi) throws IllegalArgumentException {
			// controllo tra API
			if(!ignoreApi && !this.getApi().equals(other.getApi())) 
				throw new IllegalArgumentException("Impossibile confrontare due Versioni con API diverse. Versione corrente [" + this.getApi() + "-" + this.getLabel() + "], Versione confrontata ["+ other.getApi() + "-" + other.getLabel() + "].");
