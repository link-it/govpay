...
public class LoggingOutInterceptor extends org.apache.cxf.ext.logging.LoggingOutInterceptor {
	...
	public class LoggingCallback implements CachedOutputStreamCallback {
		...
		public void onClose(CachedOutputStream cos) {
			...
			if(event.getPayload() != null)
				ctx.getContext().getResponse().setOutSize(new Long(event.getPayload().getBytes().length));
			else
				ctx.getContext().getResponse().setOutSize(0l);
			
			ctx.getTransaction().getClient().setTransportResponseCode(event.getResponseCode());
			try {
