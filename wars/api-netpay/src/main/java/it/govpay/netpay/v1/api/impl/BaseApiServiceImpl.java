...
//				.type(MediaType.APPLICATION_JSON).entity(respKo);
		this.handleEventoFail(responseBuilder, transactionId, EsitoOperazione.INTERNAL.toString(), e.getMessage(), e);
...
//		respKo.setDettaglio(e.getDetails());
		...
		String sottotipoEsito = e.getCode();
		if(e instanceof NotAuthenticatedException || e instanceof NotAuthorizedException) {
			...
			sottotipoEsito = "AUTORIZZAZIONE"; //CategoriaEnum.AUTORIZZAZIONE.name();
		} else {
			this.log.info("Errore ("+e.getClass().getSimpleName()+") durante "+methodName+": "+ e.getMessage());
		}

//		String respJson = this.getRespJson(respKo);
		ResponseBuilder responseBuilder = Response.status(e.getTransportErrorCode());
//				.type(MediaType.APPLICATION_JSON).entity(respKo);
		if(e.getTransportErrorCode() > 499)
			this.handleEventoFail(responseBuilder, transactionId, sottotipoEsito, e.getDetails(), e);
		else
			this.handleEventoKo(responseBuilder, transactionId, sottotipoEsito, e.getDetails(), e);
		return handleResponseKo(responseBuilder, transactionId).build();
	}
...
//		}
		...
		ResponseBuilder responseBuilder = Response.status(e.getStatusCode());
//		.type(MediaType.APPLICATION_JSON).entity(respKo);
		if(statusCode > 499)
			this.handleEventoFail(responseBuilder, transactionId, e.getCodEsitoV3(), e.getMessageV3(), e);
		else
			this.handleEventoKo(responseBuilder, transactionId, e.getCodEsitoV3(), e.getMessageV3(), e);
...
//		.type(MediaType.APPLICATION_JSON).entity(respKo);
		if(e.getTransportErrorCode() > 499)
			this.handleEventoFail(responseBuilder, transactionId, "SEMANTICA", e.getDetails(), e);
		else
			this.handleEventoKo(responseBuilder, transactionId, "SEMANTICA", e.getDetails(), e);
...
//		.type(MediaType.APPLICATION_JSON).entity(respKo);
		this.handleEventoKo(responseBuilder, transactionId, "SINTASSI", e.getMessage(), e);
		return handleResponseKo(responseBuilder, transactionId).build();
	}
...
//		.type(MediaType.APPLICATION_JSON).entity(respKo);
		if(e.getTransportErrorCode() > 499)
			this.handleEventoFail(responseBuilder, transactionId, e.getCode(), e.getDetails(), e);
		else
			this.handleEventoKo(responseBuilder, transactionId, e.getCode(), e.getDetails(), e);
		return handleResponseKo(responseBuilder, transactionId).build();
	}
