...
public class DominiIncassi extends MultiSelectList<Long, List<Long>>{
	
	private String incassiId= null;
	...
	public DominiIncassi(String nomeServizio,String id, String label, URI refreshUri, List<RawParamValue> paramValues,
		...
		this.incassiId = Utils.getInstance(locale).getMessageFromResourceBundle(this.nomeServizio + ".incassi.id");
	...
	protected List<Voce<Long>> getValues(List<RawParamValue> paramValues, Object... objects) throws ServiceException {
		String incassiValue = Utils.getValue(paramValues, this.incassiId);
		List<Voce<Long>> lst = new ArrayList<Voce<Long>>();

		
		if(StringUtils.isNotEmpty(incassiValue) && incassiValue.equalsIgnoreCase("false")){
			return lst;
		}
	...
	protected List<Long> getDefaultValue(List<RawParamValue> values, Object... objects) {
		String incassiValue = Utils.getValue(values, this.incassiId);
