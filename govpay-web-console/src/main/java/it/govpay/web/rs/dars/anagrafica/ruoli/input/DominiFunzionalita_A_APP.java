...
public class DominiFunzionalita_A_APP extends MultiSelectList<Long, List<Long>>{

	private String funzionalita_A_APPId= null;
	private String ruoloId = null;
	private String nomeServizio = null;
	private Servizio servizio = Servizio.Anagrafica_Applicazioni;
	private Tipo tipo = Tipo.DOMINIO;

	public DominiFunzionalita_A_APP(String nomeServizio,String id, String label, URI refreshUri, List<RawParamValue> paramValues,
			 Object... objects) {
		super(id, label, refreshUri, paramValues, objects);
		Locale locale = objects[1] != null ? (Locale) objects[1] : null;
		this.nomeServizio = nomeServizio;
		this.funzionalita_A_APPId = Utils.getInstance(locale).getMessageFromResourceBundle(this.nomeServizio + ".funzionalita_A_APP.id");
	...
	protected List<Voce<Long>> getValues(List<RawParamValue> paramValues, Object... objects) throws ServiceException {
		String funzionalita_A_APPVAlue = Utils.getValue(paramValues, this.funzionalita_A_APPId);
		List<Voce<Long>> lst = new ArrayList<Voce<Long>>();

		
		if(StringUtils.isNotEmpty(funzionalita_A_APPVAlue) && funzionalita_A_APPVAlue.equalsIgnoreCase("false")){
			return lst;
		}
	...
	protected List<Long> getDefaultValue(List<RawParamValue> values, Object... objects) {
		String funzionalita_A_APPVAlue = Utils.getValue(values, this.funzionalita_A_APPId);
