...
//					idIbanAccredito.setRequired(false);
				...
				idIbanAccreditoPostale.setValues(listaIban);
				idIbanAccreditoPostale.setDefaultValue(entry.getIdIbanAppoggio());
				sezioneRoot.addField(idIbanAccreditoPostale);
	...
	public Dettaglio getDettaglio(long id, UriInfo uriInfo, BasicBD bd) throws WebApplicationException,ConsoleException {
		...
		try{
			...
			if(tributo.getIdIbanAppoggio() != null){
				IbanAccredito ibanAppoggio = tributo.getIbanAppoggio(bd);
				root.addVoce(Utils.getInstance(this.getLanguage()).getMessageFromResourceBundle(this.nomeServizio + ".idIbanAccreditoPostale.label"), ibanAppoggio.getCodIban());
			}
	...
	public void checkEntry(Tributo entry, Tributo oldEntry) throws ValidationException {
		...
		if(!entry.getCodTributo().equals(it.govpay.model.Tributo.BOLLOT) && (entry.getIdIbanAccredito() == null && entry.getIdIbanAppoggio() == null)) {
