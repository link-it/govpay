...
public class Mail {
	...
	Logger log = LogManager.getLogger(Mail.class);
	private BasicBD bd;
	...
	public void notificaMail() throws GovPayException {
		...
		try {
			...
			AbstractFilter filter = new AbstractFilter(bd.getServiceManager().getMailServiceSearch()) {
	...
	public it.govpay.bd.model.Mail getMail(final TipoMail tipoMail, final long bundleKey) throws GovPayException, NotFoundException, MultipleResultException{
		try {
			...
			AbstractFilter filter = new AbstractFilter(this.bd.getServiceManager().getMailServiceSearch()) {
	...
	public it.govpay.bd.model.Mail generaNotificaRPT(Ente ente, Versamento versamento, Rpt rpt) throws GovPayException{
		...
		if(destinatario == null) {
			this.log.warn("Versante e debitore non riportano l'indirizzo email nell'anagrafica per la spedizione della mail RPT");
		} else {
