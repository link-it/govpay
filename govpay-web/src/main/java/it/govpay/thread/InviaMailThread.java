...
public class InviaMailThread implements Runnable {
	...
	public void run() {
		...
		try {
			...
			this.log.error("Impossibile stabilire una connessione con il database", e);
			return;
		}
		MailBD mailBD = null;
		...
		try {
			...
			try {
				...
				this.log.error("Errore durante la spedizione della this.mail [idMail: " + this.mail.getId() + "]: " + e.getMessage(), e);
				return;
			}
		} catch(Exception e) {
			this.log.error("Errore durante la spedizione della this.mail [idMail: " + this.mail.getId() + "]: " + e.getMessage(), e);
