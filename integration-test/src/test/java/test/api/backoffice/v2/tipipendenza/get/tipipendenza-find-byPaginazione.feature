Feature: Ricerca tipiPendenza con i parametri di paginazione (console-api v2)

# Corrispettivo v2 di
# test/api/backoffice/v1/tipipendenza/get/tipipendenza-find-byMetadatiPaginazione.feature.
...
Scenario: Ricerca tipiPendenza senza parametri
...
Scenario: Ricerca tipiPendenza con total true
...
Scenario: Ricerca tipiPendenza con total false
...
Scenario: Ricerca tipiPendenza con limit
...
Scenario: Ricerca tipiPendenza con page
...
Scenario: Ricerca tipiPendenza ordinata

Given url consoleBaseurl
And path 'tipiPendenza'
And param sort = 'idTipoPendenza'
And headers basicAutenticationHeader
When method get
Then status 200
And match response.results == '#[_ > 1]'
* def primoAscendente = response.results[0].idTipoPendenza
...
When method get
Then status 200
* def primoDiscendente = response.results[0].idTipoPendenza

And match primoAscendente != primoDiscendente

Scenario: Ricerca tipiPendenza senza autenticazione
