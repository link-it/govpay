Feature: Ricerca operatori con i parametri di paginazione (console-api v2)

# Corrispettivo v2 di
# test/api/backoffice/v1/operatori/get/operatori-find-byMetadatiPaginazione.feature.
...
Scenario: Ricerca operatori senza parametri
...
Scenario: Ricerca operatori con total true
...
Scenario: Ricerca operatori con total false
...
Scenario: Ricerca operatori con limit
...
Scenario: Ricerca operatori con page
...
Scenario: Ricerca operatori ordinata

Given url consoleBaseurl
And path 'operatori'
And param sort = 'principal'
And headers basicAutenticationHeader
When method get
Then status 200
And match response.results == '#[_ > 1]'
* def primoAscendente = response.results[0].principal
...
When method get
Then status 200
* def primoDiscendente = response.results[0].principal

And match primoAscendente != primoDiscendente

Scenario: Ricerca operatori senza autenticazione
