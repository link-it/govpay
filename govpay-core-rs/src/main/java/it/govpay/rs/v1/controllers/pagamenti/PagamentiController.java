...
import it.govpay.core.rs.v1.beans.pagamenti.ListaPagamentiPortale;
		...
		try{
			...
			if(!linkListe) {
				...
				for(LeggiPendenzaDTOResponse ricevutaDTOResponse: pagamentoPortaleDTOResponse.getListaPendenze()) {
					PendenzaIndex rsModel = PendenzeConverter.toRsModelIndex(ricevutaDTOResponse.getVersamento(), ricevutaDTOResponse.getUnitaOperativa(), ricevutaDTOResponse.getApplicazione(), ricevutaDTOResponse.getDominio(), ricevutaDTOResponse.getLstSingoliVersamenti(),true);
...
//			String principal = this.getPrincipal();
			...
			List<it.govpay.core.rs.v1.beans.pagamenti.Pagamento> results = new ArrayList<it.govpay.core.rs.v1.beans.pagamenti.Pagamento>();
