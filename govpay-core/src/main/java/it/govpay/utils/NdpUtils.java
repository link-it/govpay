...
public class NdpUtils {
	...
	public static void init() throws JAXBException, SAXException {
		if(jaxbContext == null || RPT_RT_schema==null) {
			...
			RPT_RT_schema = schemaFactory.newSchema(new StreamSource(NdpUtils.class.getResourceAsStream("/xsd/Merge.xsd"))); 
			jaxbContext = JAXBContext.newInstance("it.gov.digitpa.schemas._2011.pagamenti:it.gov.digitpa.schemas._2011.ws.paa:" +InformativaControparte.class.getPackage().getName());
	...
	public static CtRichiestaPagamentoTelematico buildRPT(Intermediario intermediario, Dominio dominio, Ente ente, Portale portale, Versamento versamento, Rpt rpt) throws GovPayException {
		...
		ctDominio.setIdentificativoDominio(dominio.getCodDominio());
		ctDominio.setIdentificativoStazioneRichiedente(portale.getCodPortale());
		...
		ctRpt.setEnteBeneficiario(buildEnteBeneficiario(ente.getAnagraficaEnte()));
	...
	private static CtDatiVersamentoRPT buildDatiVersamento(Rpt rpt, Versamento versamento) throws GovPayException {
		...
		datiVersamento.setIdentificativoUnivocoVersamento(versamento.getIuv());
	...
	public static String buildCodMsd() {
		String id = UUID.randomUUID().toString().replaceAll("-", "");
		if(id.length() > 35) id = id.substring(0, 35);
		return id;
	}
	
	public static void setThreadContextNdpParams(String idDominio, String iuv, String ccp, String idPsp, String tipoVersamento, String idStazione, String idCanale, String parametriSpecificiInterfaccia, String soapAction, String soggettoGovPay){
		ThreadContext.put(it.govpay.bd.model.Evento.ID_DOMINIO, idDominio);
		ThreadContext.put(it.govpay.bd.model.Evento.IUV, iuv);
		ThreadContext.put(it.govpay.bd.model.Evento.CCP, ccp);
		ThreadContext.put(it.govpay.bd.model.Evento.ID_PSP, idPsp);
		ThreadContext.put(it.govpay.bd.model.Evento.TIPO_VERSAMENTO, tipoVersamento);
		ThreadContext.put(it.govpay.bd.model.Evento.COMPONENTE, it.govpay.bd.model.Evento.COMPONENTE_WGovPay);
		ThreadContext.put(it.govpay.bd.model.Evento.ID_STAZIONE, idStazione);
		ThreadContext.put(it.govpay.bd.model.Evento.CANALE, idCanale);
		ThreadContext.put(it.govpay.bd.model.Evento.PARAMATERI_SPECIFICI_INTERFACCIA, parametriSpecificiInterfaccia);
		ThreadContext.put(it.govpay.bd.model.Evento.GP_SOGGETTO, soggettoGovPay);
		ThreadContext.put(it.govpay.bd.model.Evento.SOAP_ACTION, soapAction);
	}
