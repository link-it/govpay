...
public class ConfigurationUtils {
	...
	public ConfigurationUtils(String filePropName) throws GovPayException{
		...
		try {
			...
			logger.info("GovPay properties: " + new File(propertiesFilePath).getAbsolutePath());
		} catch (Exception ex) {
			try {
				configProperties.load(Thread.currentThread().getContextClassLoader().getResourceAsStream("/" + filePropName));
				logger.info("GovPay properties: " + Thread.currentThread().getContextClassLoader().getResource("/" + filePropName).getPath());
			} catch (Exception exx) {
				try {
					configProperties.load(ConfigurationUtils.class.getResourceAsStream("/" + filePropName));
					logger.info("GovPay properties: " + ConfigurationUtils.class.getResource("/" + filePropName).getPath());
