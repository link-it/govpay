...
public class NodoPerPa extends BasicClient {
	...
	public NodoPerPa(Intermediario intermediario) throws GovPayException {
		...
		if(objectFactory == null || log == null || eventLoggingHandlerUtils == null || messageLoggingHandlerUtils == null){
			objectFactory = new ObjectFactory();
			eventLoggingHandlerUtils = new EventLoggingHandlerUtils(true);
			messageLoggingHandlerUtils = new MessageLoggingHandlerUtils();
		}
	}
	...
	public Object send(String azione, JAXBElement<?> body, Object header) throws GovPayException {
		...
		try { 
			...
			byte[] msg = SOAPUtils.toMessage(body, header);
			
			this.messageLoggingHandlerUtils.logToSystemOut(true, connection.getRequestProperties(), msg);
			this.eventLoggingHandlerUtils.handleMessage(true,  null);
			connection.getOutputStream().write(msg);
			responseCode = connection.getResponseCode();
		} catch (Exception e) {
			this.eventLoggingHandlerUtils.handleFault();
			throw new GovPayException(GovPayExceptionEnum.ERRORE_NDP, "Errore nell'invocazione del Nodo dei Pagamenti.", e);
		}
		if(responseCode < 300) {
			try {
				...
				byte[] msg = connection.getInputStream() != null ? IOUtils.toByteArray(connection.getInputStream()) : new byte[]{};

				JAXBElement<?> jaxbElement = SOAPUtils.toJaxb(msg);
				//TODO come capire esito da risposta?
				String esito = ((Risposta)jaxbElement.getValue()).getFault() == null ? "OK" : "KO";
				
				this.eventLoggingHandlerUtils.handleMessage(false, esito);
				this.messageLoggingHandlerUtils.logToSystemOut(false, connection.getHeaderFields(), msg);
				return jaxbElement;
			} catch (Exception e) {
				log.error("Messaggio di risposta non valido: " + e, e);
				throw new GovPayException(GovPayExceptionEnum.ERRORE_NDP, "Impossibile acquisire la risposta del Nodo dei Pagamenti.", e);
			}
		} else {
			log.error("Errore nell'invocazione del Nodo dei Pagamenti: [HTTP Response Code " + responseCode + "]");
			try {
				byte[] msg = connection.getErrorStream() != null ? IOUtils.toByteArray(connection.getErrorStream()) : new byte[]{};
				this.eventLoggingHandlerUtils.handleMessage(false, "KO");
				this.messageLoggingHandlerUtils.logToSystemOut(false, connection.getHeaderFields(), msg);
				if(log.getLevel().isMoreSpecificThan(Level.DEBUG));
					log.debug("HTTP Response message: " + new String(msg));
			} catch (IOException e) {
				log.error("Impossibile serializzare l'ErrorStream della risposta: " + e);
			}
			throw new GovPayException(GovPayExceptionEnum.ERRORE_NDP, "Errore nell'invocazione del Nodo dei Pagamenti: [HTTP Response Code " + responseCode + "]");
		}
	}
