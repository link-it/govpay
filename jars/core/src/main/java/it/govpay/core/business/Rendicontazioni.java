		...
		try {
			...
			for(Stazione stazione : lstStazioni) {
				...
				for(Dominio dominio : lstDomini) { 
					log.info("Richiesta lista dei flussi di rendicontazione per il dominio [{}] ...",	dominio.getCodDominio());
				...
				if(dir.exists() && dir.isDirectory()) {
					...
					log.debug("Cartella di acquisizione FR non presente: {}{}input{}fr", GovpayConfig.getInstance().getResourceDir(), File.separatorChar, File.separatorChar);
				}
				...
				for(RendicontazioneScaricata rnd : flussiDaPagoPA) {
					...
					try {
						...
						log.debug("Flusso di rendicontazione [{}, {}, {}] gia'' acquisito", rnd.getCodDominio(), idRendicontazione.getIdentificativoFlusso(), idRendicontazione.getDataOraFlusso());
						...
						if(!keys.contains(rnd.getCodDominio() + idRendicontazione.getIdentificativoFlusso() + idRendicontazione.getDataOraFlusso().getTime())) {
							log.info("Flusso di rendicontazione [{}, {}, {}] da acquisire", rnd.getCodDominio(), idRendicontazione.getIdentificativoFlusso(), idRendicontazione.getDataOraFlusso());
				...
				for(RendicontazioneScaricata rnd : flussiDaAcquisire) {
					...
					try {
						...
						log.info("Ricevuto flusso rendicontazione per {} singoli pagamenti", flussoRendicontazione.getDatiSingoliPagamentis().size());
						...
						for(CtDatiSingoliPagamenti dsp : flussoRendicontazione.getDatiSingoliPagamentis()) {
							...
							log.info("Rendicontato (Esito {}) per un importo di ({}) [CodDominio: {}] [Iuv: {}][Iur: {}]",
							...
							try {
								...
								try {
									try {
										log.info("Pagamento [Dominio:{} Iuv:{} Iur:{} Indice:{}] non trovato, ricerco pendenza a cui assegnare la rendicontazione...",	codDominio, iuv, iur, indiceDati);
								...
								if(versamento != null) {
									log.info("Trovata Pendenza [{}, {}], verra' associata alla rendicontazione [Dominio:{} Iuv:{} Iur:{} Indice:{}].", versamento.getApplicazione(configWrapper).getCodApplicazione(), versamento.getCodVersamentoEnte(), codDominio, iuv, iur, indiceDati);
									...
									log.info("Non e' stata trovata nessuna pendenza corrispondente alla rendicontazione [Dominio:{} Iuv:{} Iur:{} Indice:{}]: {}.", codDominio, iuv, iur, indiceDati, erroreVerifica);
								}
...
//											continue;
								...
								log.info("Pagamento [Dominio:{} Iuv:{} Iur:{} Indice:{}] rendicontato con errore: il pagamento non risulta presente in base dati.", codDominio, iuv, iur, indiceDati);
								...
								log.info("Pagamento rendicontato duplicato: [Dominio:{} Iuv:{} Iur:{} Indice:{}]",	codDominio, iuv, iur, indiceDati);
								...
								if(fr.getRendicontazioni() != null) {
									for (Rendicontazione r2 : fr.getRendicontazioni()) {
										if(r2.getIuv().equals(rendicontazione.getIuv()) 
											...
											log.info("Rendicontazione [Dominio:{} Iuv:{} Iur:{} Indice:{}] duplicata all''interno del flusso, in violazione delle specifiche PagoPA. Necessario intervento manuale per la risoluzione del problema.", codDominio, iuv, iur, indiceDati);
						...
						if(totaleImportiRendicontati.compareTo(flussoRendicontazione.getImportoTotalePagamenti()) != 0){
							...
							log.info("La somma degli importi rendicontati [{}] non corrisponde al totale indicato nella testata del flusso [{}]", totaleImportiRendicontati, flussoRendicontazione.getImportoTotalePagamenti());
						...
						try {
							if(flussoRendicontazione.getDatiSingoliPagamentis().size() != flussoRendicontazione.getNumeroTotalePagamenti().longValueExact()) {
								...
								log.info("Il numero di pagamenti rendicontati [{}] non corrisponde al totale indicato nella testata del flusso [{}]", flussoRendicontazione.getDatiSingoliPagamentis().size(), flussoRendicontazione.getNumeroTotalePagamenti().longValueExact());
							...
							ctx.getApplicationLogger().log("rendicontazioni.numeroRendicontazioniErrato");
							log.info("Il numero di pagamenti rendicontati [{}] non corrisponde al totale indicato nella testata del flusso [????]", flussoRendicontazione.getDatiSingoliPagamentis().size());
						...
						try {
							...
							try {
								...
								if(frEsistente.getDataFlusso().before(fr.getDataFlusso())) {
									...
									log.info("Trovata versione precedente [{}] da marcare come obsoleta.", fr.getCodFlusso());
									...
									log.info("Trovata versione successiva [{}]. Il nuovo flusso viene marcato come obsoleto.", fr.getCodFlusso());
									fr.setObsoleto(true);
								}
							} catch (NotFoundException e) {
	...
	private List<TipoIdRendicontazione> chiediListaFr(Stazione stazione, Dominio dominio, Giornale giornale) throws UtilsException{ 
		...
		try {
			...
			try {
				...
				log.debug("Richiesta elenco flussi rendicontazione per il dominio [{}] al nodo in corso...", dominio.getCodDominio());
				...
				eventoCtx.setEsito(Esito.OK);
				log.debug("Richiesta elenco flussi rendicontazione per il dominio [{}] al nodo completata.", dominio.getCodDominio());
			...
			if(risposta.getFault() != null) {
				...
				if(risposta.getElencoFlussiRendicontazione() == null || risposta.getElencoFlussiRendicontazione().getTotRestituiti() == 0) {
					log.info("Richiesta elenco flussi rendicontazione per il dominio [{}]: ritornata lista vuota dal psp", dominio.getCodDominio());
				...
				log.info("Richiesta elenco flussi rendicontazione per il dominio [{}]: ritornati {} flussi.", dominio.getCodDominio(), risposta.getElencoFlussiRendicontazione().getTotRestituiti());
