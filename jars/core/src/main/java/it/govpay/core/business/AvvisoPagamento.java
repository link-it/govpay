...
public class AvvisoPagamento {
	...
	public void cancellaAvviso(Versamento versamento) throws GovPayException { 
		...
		try {
			log.debug("Delete Avviso Pagamento per la pendenza [IDA2A: {} | Id: {}]", versamento.getApplicazione(configWrapper).getCodApplicazione(), versamento.getCodVersamentoEnte());
		...
		try {
			...
			if(avviso == null) {
				try {
					...
					if(printAvviso.isSalvaSuDB()) {
						log.debug("Creazione PDF Avviso Pagamento [Dominio: {} | IUV: {}] Salvataggio su DB...",	printAvviso.getCodDominio(), printAvviso.getIuv());
						avvisiBD.insertStampa(avviso);
						log.debug("Creazione PDF Avviso Pagamento [Dominio: {} | IUV: {}] Salvataggio su DB completato.", printAvviso.getCodDominio(), printAvviso.getIuv());
					}
				...
				try {
					...
					if(printAvviso.isSalvaSuDB()) {
						log.debug("Aggiornamento PDF Avviso Pagamento [Dominio: {} | IUV: {}] Salvataggio su DB...", printAvviso.getCodDominio(), printAvviso.getIuv());
						avvisiBD.updatePdfStampa(avviso);
						log.debug("Aggiornamento PDF Avviso Pagamento [Dominio: {} | IUV: {}] Salvato.", printAvviso.getCodDominio(), printAvviso.getIuv());
					}
				} catch (UtilsException | JAXBException | IOException | JRException | PropertyNotFoundException | InvalidSwitchValueException e) {
			...
			log.debug("Stampa PDF Avviso Pagamento [IDA2A: {} | IdPendenza: {}]  Creazione Stampa completata.",	applicazione.getCodApplicazione(), printAvviso.getVersamento().getCodVersamentoEnte());
	...
	private byte[] getBytesAvvisoVersamento(PrintAvvisoVersamentoDTO printAvviso, boolean update) throws ServiceException, JAXBException, IOException, JRException, UtilsException, PropertyNotFoundException, InvalidSwitchValueException {
		...
		log.debug("{} PDF Avviso Pagamento [Dominio: {} | IUV: {}] Lettura properties...", logPrefix, printAvviso.getCodDominio(), printAvviso.getIuv());
		AvvisoPagamentoProperties avProperties = AvvisoPagamentoProperties.getInstance();
		log.debug("{} PDF Avviso Pagamento [Dominio: {} | IUV: {}] Lettura properties completata.",	logPrefix, printAvviso.getCodDominio(), printAvviso.getIuv());
		...
		if(linguaSelezionata != null) {
			log.debug("{} PDF Avviso Pagamento Multilingua [Dominio: {} | IUV: {}] Creazione input...", logPrefix, printAvviso.getCodDominio(), printAvviso.getIuv());
			it.govpay.stampe.model.v2.AvvisoPagamentoInput input = AvvisoPagamentoV2Utils.fromVersamento(printAvviso, linguaSelezionata);
			log.debug("{} PDF Avviso Pagamento Multilingua [Dominio: {} | IUV: {}] Creazione input completata.", logPrefix, printAvviso.getCodDominio(), printAvviso.getIuv());

			log.debug("{} PDF Avviso Pagamento Multilingua [Dominio: {} | IUV: {}] Generazione pdf...",	logPrefix, printAvviso.getCodDominio(), printAvviso.getIuv());
			pdfBytes = AvvisoPagamentoPdf.getInstance().creaAvvisoV2(log, input, printAvviso.getCodDominio(), avProperties);
			log.debug("{} PDF Avviso Pagamento Multilingua [Dominio: {} | IUV: {}] Generazione pdf completata.", logPrefix, printAvviso.getCodDominio(), printAvviso.getIuv());
		} else {
			log.debug("{} PDF Avviso Pagamento [Dominio: {} | IUV: {}] Creazione input...", logPrefix, printAvviso.getCodDominio(), printAvviso.getIuv());
			AvvisoPagamentoInput input = AvvisoPagamentoUtils.fromVersamento(printAvviso);
			log.debug("{} PDF Avviso Pagamento [Dominio: {} | IUV: {}] Creazione input completata.", logPrefix, printAvviso.getCodDominio(), printAvviso.getIuv());

			log.debug("{} PDF Avviso Pagamento [Dominio: {} | IUV: {}] Generazione pdf...", logPrefix, printAvviso.getCodDominio(), printAvviso.getIuv());
			pdfBytes = AvvisoPagamentoPdf.getInstance().creaAvviso(log, input, printAvviso.getCodDominio(), avProperties);
			log.debug("{} PDF Avviso Pagamento [Dominio: {} | IUV: {}] Generazione pdf completata.", logPrefix, printAvviso.getCodDominio(), printAvviso.getIuv());
		}
	...
	private LinguaSecondaria getSecondaLingua(LinguaSecondaria linguaSecondaria, Versamento versamento)  {
		...
		if(linguaSecondaria != null) {
			if(linguaSecondaria.equals(LinguaSecondaria.FALSE)) {
				log.debug("Lingua secondaria decisa sovrascrivendo il valore di default impostato nella pendenza: ricevuto valore [{}], l''avviso verra'' stampato solo in italiano.", linguaSecondaria);
				linguaSelezionata = null;
			} else {
				log.debug("Lingua secondaria decisa sovrascrivendo il valore di default impostato nella pendenza: ricevuto valore [{}], l''avviso verra'' stampato in formato bilingue.", linguaSecondaria);
			...
			if(linguaSecondariaDefault != null) {			
				if(linguaSecondariaDefault.equals(LinguaSecondaria.FALSE)) {
					log.debug("Lingua secondaria decisa utilizzando il valore di default impostato nella pendenza: [{}], l''avviso verra'' stampato solo in italiano.", linguaSecondariaDefault);
					linguaSelezionata = null;
				} else {
					log.debug("Lingua secondaria decisa utilizzando il valore di default impostato nella pendenza: [{}], l''avviso verra'' stampato in formato bilingue.", linguaSecondariaDefault);
		...
		if(linguaSelezionata != null) {
			...
			try {
				...
				if(labelsLingua == null || labelsLingua.isEmpty()) {
					log.debug("Non sono state definite le label per la lingua secondaria [{}], l''avviso verra'' stampato solo in italiano.", linguaSelezionata);
					linguaSelezionata = null;
				}
	...
	public PrintAvvisoDTOResponse printAvvisoDocumento(PrintAvvisoDocumentoDTO printAvviso) throws ServiceException, UnprocessableEntityException{ 
		...
		try {
			...
			if(printAvviso.isSalvaSuDB()) {
				try {
					log.debug("Lettura PDF Avviso Documento [IDA2A: {} | CodDocumento: {}] Check Esistenza DB...", applicazione.getCodApplicazione(), printAvviso.getDocumento().getCodDocumento());
					avviso = avvisiBD.getAvvisoDocumento(printAvviso.getDocumento().getId());
					log.debug("Lettura PDF Avviso Documento [IDA2A: {} | CodDocumento: {}] trovato].", applicazione.getCodApplicazione(), printAvviso.getDocumento().getCodDocumento());
				}catch (NotFoundException e) {
					log.debug("Lettura PDF Avviso Documento [IDA2A: {} | CodDocumento: {}] non trovato].", applicazione.getCodApplicazione(), printAvviso.getDocumento().getCodDocumento());
				}
			}
			// se non c'e' allora vien inserito
			...
			if(avviso == null) {
				try {
					...
					avviso.setPdf(pdfBytes);
					if(printAvviso.isSalvaSuDB()) {
						log.debug("Creazione PDF Avviso Documento [IDA2A: {} | CodDocumento: {}] Salvataggio su DB...", applicazione.getCodApplicazione(), printAvviso.getDocumento().getCodDocumento());
						avvisiBD.insertStampa(avviso);
						log.debug("Creazione PDF Avviso Documento [IDA2A: {} | CodDocumento: {}] Salvataggio su DB completato.", applicazione.getCodApplicazione(), printAvviso.getDocumento().getCodDocumento());
					}
				} catch (UtilsException | JAXBException | IOException | JRException| PropertyNotFoundException | InvalidSwitchValueException e) {
				...
				try {
					...
					avviso.setPdf(pdfBytes);
					if(printAvviso.isSalvaSuDB()) {
						log.debug("Aggiornamento PDF Avviso Documento [IDA2A: {} | CodDocumento: {}] Salvataggio su DB...", applicazione.getCodApplicazione(), printAvviso.getDocumento().getCodDocumento());
						avvisiBD.updatePdfStampa(avviso);
						log.debug("Aggiornamento PDF Avviso Documento [IDA2A: {} | CodDocumento: {}] Salvataggio su DB completato.", applicazione.getCodApplicazione(), printAvviso.getDocumento().getCodDocumento());
					}
				} catch (UtilsException | JAXBException | IOException | JRException| PropertyNotFoundException | InvalidSwitchValueException e) {
			...
			log.debug("Stampa PDF Avviso Pagamento Documento [IDA2A: {} | CodDocumento: {}] Creazione Stampa completata.", applicazione.getCodApplicazione(), printAvviso.getDocumento().getCodDocumento());
			response.setAvviso(avviso);
		}finally {
			if(avvisiBD != null)
				avvisiBD.closeConnection();
	...
	private byte[] getBytesAvvisoDocumento(PrintAvvisoDocumentoDTO printAvviso, Applicazione applicazione, Dominio dominio, boolean update) throws ServiceException, UnprocessableEntityException, JAXBException, IOException, JRException,	UtilsException, PropertyNotFoundException, InvalidSwitchValueException {
		...
		log.debug("{} PDF Avviso Documento [IDA2A: {} | CodDocumento: {}] Lettura properties...", logPrefix, applicazione.getCodApplicazione(), printAvviso.getDocumento().getCodDocumento());
		AvvisoPagamentoProperties avProperties = AvvisoPagamentoProperties.getInstance();
		log.debug("{} PDF Avviso Documento [IDA2A: {} | CodDocumento: {}] Lettura properties completata.", logPrefix, applicazione.getCodApplicazione(), printAvviso.getDocumento().getCodDocumento());
		...
		byte[] pdfBytes = null;
		if(linguaSelezionata != null) {
			log.debug("{} PDF Avviso Documento Multilingua [IDA2A: {} | CodDocumento: {}] Creazione input...", logPrefix, applicazione.getCodApplicazione(), printAvviso.getDocumento().getCodDocumento());
			...
			log.debug("{} PDF Avviso Documento Multilingua [IDA2A: {} | CodDocumento: {}] Creazione input completata.", logPrefix, applicazione.getCodApplicazione(), printAvviso.getDocumento().getCodDocumento());

			log.debug("{} PDF Avviso Documento Multilingua [IDA2A: {} | CodDocumento: {}] Generazione pdf...", logPrefix, applicazione.getCodApplicazione(), printAvviso.getDocumento().getCodDocumento());
			pdfBytes = AvvisoPagamentoPdf.getInstance().creaAvvisoV2(log, input, dominio.getCodDominio(), avProperties);
			log.debug("{} PDF Avviso Documento Multilingua [IDA2A: {} | CodDocumento: {}] Generazione pdf completata.", logPrefix, applicazione.getCodApplicazione(), printAvviso.getDocumento().getCodDocumento());
		} else {
			log.debug("{} PDF Avviso Documento [IDA2A: {} | CodDocumento: {}] Creazione input...", logPrefix, applicazione.getCodApplicazione(), printAvviso.getDocumento().getCodDocumento());
			AvvisoPagamentoInput input = AvvisoPagamentoUtils.fromDocumento(printAvviso, versamenti, log);
			log.debug("{} PDF Avviso Documento [IDA2A: {} | CodDocumento: {}] Creazione input completata.", logPrefix, applicazione.getCodApplicazione(), printAvviso.getDocumento().getCodDocumento());

			log.debug("{} PDF Avviso Documento [IDA2A: {} | CodDocumento: {}] Generazione pdf...", logPrefix, applicazione.getCodApplicazione(), printAvviso.getDocumento().getCodDocumento());
			pdfBytes = AvvisoPagamentoPdf.getInstance().creaAvviso(log, input, dominio.getCodDominio(), avProperties);
			log.debug("{} PDF Avviso Documento [IDA2A: {} | CodDocumento: {}] Generazione pdf completata.", logPrefix, applicazione.getCodApplicazione(), printAvviso.getDocumento().getCodDocumento());
		}
