...
public class PagamentoRendicontazioneBD extends BasicBD {
	...
	public PagamentoRendicontazioneBD(BasicBD basicBD) {
		super(basicBD);
	}
...
//			lstReturnType.add(Pagamento.model().IBAN_ACCREDITO.getFieldType());
			...
			String initialNativeQuery = NativeQueries.getInstance().getPagamentoRendicontazioneQuery();
	...
	private RendicontazionePagamento getRendicontazionePagamento(List<Object> record) throws ServiceException {
		...
		Object idRendicontazione = record.get(49);
		
		boolean existsRendicontazione = idRendicontazione != null;
		
		Object idVersamento = record.get(83);
		boolean existsVersamento = idVersamento != null;

		boolean existsFr = false;
		if(existsRendicontazione) {
			Object idFr = record.get(11);
			existsFr = idFr != null;
		}
		...
		if(existsVersamento) {
			...
			versamento.setImportoTotale(BigDecimal.valueOf((Double) record.get(i++)));
		...
		if(existsRendicontazione) {
			...
			rendicontazione.setImporto(BigDecimal.valueOf((Double) record.get(i++)));
		...
		pagamento.setImportoPagato(BigDecimal.valueOf((Double) record.get(i++)));
		...
		if(record.get(i) != null) {
			pagamento.setCommissioniPsp(BigDecimal.valueOf((Double) record.get(i++)));
		} else {
			i++;
		}
		...
		if(record.get(i) != null) {
			pagamento.setImportoRevocato(BigDecimal.valueOf((Double) record.get(i++)));
		} else {
			i++;
		}
		...
		if(existsVersamento) {
			...
			singoloVersamento.setImportoSingoloVersamento(BigDecimal.valueOf((Double) record.get(i++)));
			if(record.get(i) != null) {
