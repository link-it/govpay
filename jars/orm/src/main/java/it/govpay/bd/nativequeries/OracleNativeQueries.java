...
public class OracleNativeQueries extends NativeQueries {
	...
	public String getRendicontazionePagamentoQuery() {
		return "SELECT " +
				...
				" sv_cod_singolo_versamento_ente,sv_stato_singolo_versamento,sv_importo_singolo_versamento,sv_tipo_bollo,sv_hash_documento,sv_provincia_residenza,sv_tipo_contabilita,sv_codice_contabilita,sv_dati_allegati,sv_id,sv_id_versamento,sv_id_tributo,sv_id_iban_accredito,tipo, inc_data_ora_incasso " +
				" FROM (SELECT " +
				...
				" s1.sv_cod_singolo_versamento_ente,s1.sv_stato_singolo_versamento,s1.sv_importo_singolo_versamento,s1.sv_tipo_bollo,s1.sv_hash_documento,s1.sv_provincia_residenza,s1.sv_tipo_contabilita,s1.sv_codice_contabilita,s1.sv_dati_allegati,s1.sv_id,s1.sv_id_versamento,s1.sv_id_tributo,s1.sv_id_iban_accredito, s1.tipo , incassi.data_ora_incasso as inc_data_ora_incasso,  ROW_NUMBER() OVER ( ORDER BY p_data_pagamento)  AS rowNumber" +
				" FROM " +
				" (( " +
	...
	public String getUpdateVersamentiPerDominioConLimit() {
		return "UPDATE versamenti set id_tracciato = ? where id in (select id FROM (select id, ROW_NUMBER() OVER ( ORDER BY id) AS rowNumber from versamenti where id_dominio = ? and da_avvisare = ? and modo_avvisatura = ? and id_tracciato IS null) WHERE ( rowNumber > 0 AND rowNumber <= ? ))";
	}
