	...
	public IExpression _toSimpleSearchExpression() throws ServiceException {
		try {
			...
			if(this.idDomini != null){
				...
				VistaVersamentoFieldConverter converter = new VistaVersamentoFieldConverter(ConnectionManager.getJDBCServiceManagerProperties().getDatabase()); 
				CustomField cf = new CustomField("id_dominio", Long.class, "id_dominio", converter.toTable(VistaVersamento.model().ID_UO));
				newExpressionDomini.in(cf, this.idDomini);
				newExpressionDomini.isNotNull(VistaVersamento.model().ID_UO.COD_UO); //Sempre not null, solo per forzare la join
	...
	public IExpression _toExpression() throws ServiceException {
		try {
			...
			if(this.statiVersamento != null && this.statiVersamento.size() > 0){
				...
				for (StatoVersamento statoVersamento : statiVersamento) {
					...
					case INCASSATO:
						...
						orStatoIncassato.in(VistaVersamento.model().STATO_VERSAMENTO, StatoVersamento.ESEGUITO.toString(),StatoVersamento.ESEGUITO_SENZA_RPT.toString());
						orStatoIncassato.and().equals(VistaVersamento.model().STATO_PAGAMENTO, statoVersamento.toString());
						orStati.add(orStatoIncassato);
						break;
					case ESEGUITO:
						IExpression orStatoEseguito = this.newExpression();
						orStatoEseguito.equals(VistaVersamento.model().STATO_VERSAMENTO, StatoVersamento.ESEGUITO.toString());
						orStatoEseguito.and().equals(VistaVersamento.model().STATO_PAGAMENTO, StatoPagamento.PAGATO.toString());
						orStati.add(orStatoEseguito);
						break;
					case ESEGUITO_SENZA_RPT:
						IExpression orStatoEseguitoSenzaRPT = this.newExpression();
						orStatoEseguitoSenzaRPT.equals(VistaVersamento.model().STATO_VERSAMENTO, StatoVersamento.ESEGUITO_SENZA_RPT.toString());
						orStatoEseguitoSenzaRPT.and().equals(VistaVersamento.model().STATO_PAGAMENTO, StatoPagamento.PAGATO.toString());
					...
					default:
						...
						orStato.equals(VistaVersamento.model().STATO_VERSAMENTO, statoVersamento.toString());
						orStati.add(orStato);
						break;
					}
				}
			...
			if(this.abilitaFiltroScaduto) {
				...
				newExpression.isNotNull(VistaVersamento.model().DATA_SCADENZA).and().lessEquals(VistaVersamento.model().DATA_SCADENZA, calendar.getTime());
			...
			if(this.abilitaFiltroNonScaduto) {
				...
				or1Expression.isNotNull(VistaVersamento.model().DATA_SCADENZA).and().greaterEquals(VistaVersamento.model().DATA_SCADENZA, calendar.getTime());
				or2Expression.isNull(VistaVersamento.model().DATA_SCADENZA);
			...
			if(this.dataInizio != null && this.dataFine != null) {
				...
				newExpression.between(VistaVersamento.model().DATA_CREAZIONE, this.dataInizio,this.dataFine);
				addAnd = true;
			} else {
			...
			if(this.codUnivocoDebitore != null) {
				if(addAnd)
					newExpression.and();
				newExpression.equals(VistaVersamento.model().SRC_DEBITORE_IDENTIFICATIVO, this.codUnivocoDebitore.toUpperCase());
				addAnd = true;
			}
			...
			if(this.cfCittadino!= null) {
				...
				newExpression.equals(VistaVersamento.model().SRC_DEBITORE_IDENTIFICATIVO, this.cfCittadino.toUpperCase());
			...
			if(this.idVersamento != null && !this.idVersamento.isEmpty()){
				...
				if(addAnd)
					newExpression.and();
				CustomField cf = new CustomField("id", Long.class, "id", converter.toTable(VistaVersamento.model()));
			...
			if(this.idDomini != null && !this.idDomini.isEmpty()){
				...
				if(addAnd)
					newExpression.and();
				CustomField cf = new CustomField("id_dominio", Long.class, "id_dominio", converter.toTable(VistaVersamento.model()));
			...
			if(this.idUo != null && !this.idUo.isEmpty()){
				...
				for (IdUnitaOperativa uo : this.idUo) {
					...
					if(uo.getIdUnita() != null) {
						CustomField cf = new CustomField("id_uo", Long.class, "id_uo", converter.toTable(VistaVersamento.model()));
			...
			if(this.codVersamento != null){
				...
				newExpression.equals(VistaVersamento.model().COD_VERSAMENTO_ENTE, this.codVersamento);
				addAnd = true;
			}
			...
			if(this.codApplicazione != null){
				...
				newExpression.equals(VistaVersamento.model().ID_APPLICAZIONE.COD_APPLICAZIONE, this.codApplicazione);
				addAnd = true;
			}
			...
			if(this.codDominio != null){
				...
				newExpression.equals(VistaVersamento.model().ID_DOMINIO.COD_DOMINIO, this.codDominio);
				addAnd = true;
			}
			...
			if(this.idTracciato != null) {
				...
				newExpression.and().isNotNull(VistaVersamento.model().ID_OPERAZIONE.STATO);
				CustomField cf = new CustomField("id_tracciato", Long.class, "id_tracciato", converter.toTable(VistaVersamento.model().ID_OPERAZIONE));
			...
			if(this.idTipiVersamento != null && !this.idTipiVersamento.isEmpty()){
				...
				if(addAnd)
					newExpression.and();
				CustomField cf = new CustomField("id_tipo_versamento", Long.class, "id_tipo_versamento", converter.toTable(VistaVersamento.model()));
			...
			if(this.direzione != null){
				...
				newExpression.equals(VistaVersamento.model().DIREZIONE, this.direzione);
				addAnd = true;
			}
			...
			if(this.divisione != null){
				...
				newExpression.equals(VistaVersamento.model().DIVISIONE, this.divisione);
				addAnd = true;
			}
			...
			if(this.idSessione != null){
				...
				newExpression.equals(VistaVersamento.model().ID_SESSIONE, this.idSessione);
				addAnd = true;
			}
			...
			if(this.iuv != null){
				...
				newExpression.equals(VistaVersamento.model().SRC_IUV, this.iuv.toUpperCase());
				addAnd = true;
			}
			...
			if(this.abilitaFiltroCittadino) {
				...
				IExpression orExpr = this.newExpression();
				orExpr.equals(VistaVersamento.model().TIPO, TipologiaTipoVersamento.DOVUTO.toString())
					.or().greaterThan(VistaVersamento.model().IMPORTO_PAGATO, 0);
			...
			if(this.mostraSpontaneiNonPagati != null) {
				if(!this.mostraSpontaneiNonPagati) {
					...
					IExpression orExpr = this.newExpression();
					orExpr.equals(VistaVersamento.model().TIPO, TipologiaTipoVersamento.SPONTANEO.toString())
						.and().equals(VistaVersamento.model().STATO_VERSAMENTO, StatoVersamento.NON_ESEGUITO.toString());
			...
			if(this.idDocumento != null){
				if(addAnd)
					newExpression.and();
				CustomField cf = new CustomField("id_documento", Long.class, "id_documento", converter.toTable(VistaVersamento.model()));
...
//	}	
	...
	public void addSortField(SortFields field) {
		...
		case AGGIORNAMENTO_ASC:
			filterSortWrapper.setField(VistaVersamento.model().DATA_ORA_ULTIMO_AGGIORNAMENTO); 
		...
		case CARICAMENTO_ASC:
			filterSortWrapper.setField(VistaVersamento.model().DATA_CREAZIONE); 
			filterSortWrapper.setSortOrder(SortOrder.ASC);
			break;
		...
		case SCADENZA_ASC:
			filterSortWrapper.setField(VistaVersamento.model().DATA_SCADENZA); 
			filterSortWrapper.setSortOrder(SortOrder.ASC);
			break;
		...
		case STATO_ASC:
			filterSortWrapper.setField(VistaVersamento.model().STATO_VERSAMENTO); 
			filterSortWrapper.setSortOrder(SortOrder.ASC);
			break;
...
//				sqlQueryObject.addWhereCondition(true,converter.toColumn(model.DATA_SCADENZA, true) + " >= ? ");
			...
			if(this.idTracciato != null) {
				if(!addTabellaOperazioni) {
					String versamenti = converter.toAliasTable(VistaVersamento.model());
					String operazioni = converter.toAliasTable(VistaVersamento.model().ID_OPERAZIONE);
