...
public class JDBCVistaEventiPagamentoService extends JDBCVistaEventiPagamentoServiceSearch  implements IDBVistaEventiPagamentoService {
	...
	public JDBCVistaEventiPagamentoService(JDBCServiceManager jdbcServiceManager) throws ServiceException {
		...
		String msgInit = JDBCVistaEventiPagamentoService.class.getName()+ " initialized";
		this.log.debug(msgInit);
		this.serviceCRUD = JDBCProperties.getInstance(ProjectInfo.getInstance()).getServiceCRUD("vistaEventiPagamento");
	...
	private ServiceException newServiceExceptionParameterIsNull(){
		return new ServiceException(PARAMETER_TYPE_PREFIX+Evento.class.getName()+") 'vistaEventiPagamento' is null");
	}
