...
public class JDBCDominioServiceImpl extends JDBCDominioServiceSearchImpl
	...
	public void create(JDBCServiceManagerProperties jdbcProperties, Logger log, Connection connection, ISQLQueryObject sqlQueryObject, Dominio dominio, org.openspcoop2.generic_project.beans.IDMappingBehaviour idMappingResolutionBehaviour) throws NotImplementedException,ServiceException,Exception {
		...
		sqlQueryObjectInsert.addInsertField(this.getDominioFieldConverter().toColumn(Dominio.model().COD_CONNETTORE_SEND,false),"?");
		...
		long id = jdbcUtilities.insertAndReturnGeneratedKey(sqlQueryObjectInsert, keyGenerator, jdbcProperties.isShowSql(),
			...
			new org.openspcoop2.generic_project.dao.jdbc.utils.JDBCObject(dominio.getCodConnettoreSend(),Dominio.model().COD_CONNETTORE_SEND.getFieldType()),
	...
	public void update(JDBCServiceManagerProperties jdbcProperties, Logger log, Connection connection, ISQLQueryObject sqlQueryObject, long tableId, Dominio dominio, org.openspcoop2.generic_project.beans.IDMappingBehaviour idMappingResolutionBehaviour) throws NotFoundException, NotImplementedException, ServiceException, Exception {
		...
		sqlQueryObjectUpdate.addUpdateField(this.getDominioFieldConverter().toColumn(Dominio.model().COD_CONNETTORE_MY_PIVOT,false), "?");
		lstObjects_dominio.add(new JDBCObject(dominio.getCodConnettoreMyPivot(), Dominio.model().COD_CONNETTORE_MY_PIVOT.getFieldType()));
		sqlQueryObjectUpdate.addUpdateField(this.getDominioFieldConverter().toColumn(Dominio.model().COD_CONNETTORE_SECIM,false), "?");
		...
		sqlQueryObjectUpdate.addUpdateField(this.getDominioFieldConverter().toColumn(Dominio.model().COD_CONNETTORE_SEND,false), "?");
		lstObjects_dominio.add(new JDBCObject(dominio.getCodConnettoreSend(), Dominio.model().COD_CONNETTORE_SEND.getFieldType()));
