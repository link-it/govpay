...
ALTER TABLE versamenti ADD COLUMN metadata TEXT;-- 13/02/2024 Supporto Metadata RT
DROP VIEW versamenti_incassi;
DROP VIEW v_rendicontazioni_ext;
DROP VIEW v_rpt_versamenti;
DROP VIEW v_versamenti;
...
CREATE VIEW v_vrs_non_rnd AS 
 SELECT singoli_versamenti.cod_singolo_versamento_ente AS sng_cod_sing_vers_ente,
   ...
   FROM pagamenti
     ...
     LEFT JOIN rpt ON pagamenti.id_rpt = rpt.id
     LEFT JOIN incassi ON pagamenti.id_incasso = incassi.id
  WHERE rendicontazioni.id IS NULL;CREATE VIEW versamenti_incassi AS SELECT
    versamenti.id,
    versamenti.cod_versamento_ente,
    versamenti.nome,
    versamenti.importo_totale,
