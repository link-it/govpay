...
ALTER TABLE fr ADD COLUMN data_ora_pubblicazione DATETIME(3);


-- 03/11/2025 Reso opzionale il campo xml della tabella fr
ALTER TABLE fr MODIFY COLUMN xml MEDIUMBLOB NULL;


-- 03/11/2025 Aggiunti campi data_ora_aggiornamento e revisione alla tabella fr
ALTER TABLE fr ADD COLUMN data_ora_aggiornamento DATETIME(3);
...
ALTER TABLE intermediari ADD COLUMN cod_connettore_aca VARCHAR(35);
...
ALTER TABLE fr ADD COLUMN id_dominio BIGINT;
UPDATE fr JOIN domini ON fr.cod_dominio = domini.cod_dominio SET fr.id_dominio = domini.id;
ALTER TABLE fr MODIFY id_dominio BIGINT NOT NULL;
...
-- 02/12/2025 Estensione campi ragione_sociale_psp e ragione_sociale_dominio da 70 a 255 caratteri
DROP VIEW IF EXISTS v_riscossioni;
DROP VIEW IF EXISTS v_rendicontazioni_ext;

ALTER TABLE fr MODIFY COLUMN ragione_sociale_psp VARCHAR(255);
ALTER TABLE fr MODIFY COLUMN ragione_sociale_dominio VARCHAR(255);
...
-- Aggiunta indice su domini.scarica_fr per ottimizzare query di ricerca domini abilitati
