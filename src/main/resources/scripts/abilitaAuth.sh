...
[[ $BACKOFFICE == *"basic"* ]] && { BACKOFFICE_BASIC_GP=true; BACKOFFICE_BASIC_LDAP=false; }
[[ $BACKOFFICE == *"ldap"* ]] && { BACKOFFICE_BASIC_GP=false; BACKOFFICE_BASIC_LDAP=true; }
...
[[ $PENDENZE == *"basic"* ]] && { PENDENZE_BASIC_GP=true; PENDENZE_BASIC_LDAP=false; }
[[ $PENDENZE == *"ldap"* ]] && { PENDENZE_BASIC_GP=false; PENDENZE_BASIC_LDAP=true; }
...
[[ $RAGIONERIA == *"basic"* ]] && { RAGIONERIA_BASIC_GP=true; RAGIONERIA_BASIC_LDAP=false; }
[[ $RAGIONERIA == *"ldap"* ]] && { RAGIONERIA_BASIC_GP=false; RAGIONERIA_BASIC_LDAP=true; }
...
# API-Backoffice
API_TARGET_DIR="api-backoffice"$TARGET_DIR
CURRENT_WAR="backoffice"
...
if $BACKOFFICE_API_KEY
then
  echo "API-Backoffice abilitazione ApiKey auth...";
...
# API-Pendenze
API_TARGET_DIR="api-pendenze"$TARGET_DIR
CURRENT_WAR="pendenze"
...
if $PENDENZE_API_KEY
then
  echo "API-Pendenze abilitazione ApiKey auth...";
  sed -i -e "s#API_KEY_START#API_KEY_START -->#g" $APP_CONTEXT_BASE_DIR/$API_PREFIX$CONTEXT_SECURITY_XML_SUFFIX
  sed -i -e "s#API_KEY_END#<!-- API_KEY_END#g" $APP_CONTEXT_BASE_DIR/$API_PREFIX$CONTEXT_SECURITY_XML_SUFFIX
...
# API-Ragioneria
API_TARGET_DIR="api-ragioneria"$TARGET_DIR
CURRENT_WAR="ragioneria"
...
if $RAGIONERIA_API_KEY
then
  echo "API-Ragioneria abilitazione ApiKey auth...";
  sed -i -e "s#API_KEY_START#API_KEY_START -->#g" $APP_CONTEXT_BASE_DIR/$API_PREFIX$CONTEXT_SECURITY_XML_SUFFIX
  sed -i -e "s#API_KEY_END#<!-- API_KEY_END#g" $APP_CONTEXT_BASE_DIR/$API_PREFIX$CONTEXT_SECURITY_XML_SUFFIX
...
then
  ...
  API_TARGET_DIR="api-pagopa"$TARGET_DIR
  CURRENT_WAR="pagopa"
  ...
  sed -i -e "s#BASIC_TOMCAT_START#BASIC_TOMCAT_START -->#g" $APP_CONTEXT_BASE_DIR/$API_PREFIX$CONTEXT_SECURITY_XML_SUFFIX
  sed -i -e "s#BASIC_TOMCAT_END#<!-- BASIC_TOMCAT_END#g" $APP_CONTEXT_BASE_DIR/$API_PREFIX$CONTEXT_SECURITY_XML_SUFFIX
...
if $PAGOPA_API_KEY
then
  echo "API-PagoPA abilitazione api key...";
...
then
  ...
  API_TARGET_DIR="api-jppapdp"$TARGET_DIR
  CURRENT_WAR="jppapdp"
...
if $JPPAPDP_API_KEY
then
  echo "API-Maggioli JPPA abilitazione api key...";
...
echo "Configurazione govpay*.war completata";
